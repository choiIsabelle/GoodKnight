...
export default function AnalysisBoardWrapper() {
  ...
  // Helper: allow an external, build-time configured backend URL for
  // environments where the local dev Python server isn't available
  // (e.g. Cloudflare Pages -> Render). Set NEXT_PUBLIC_REMOTE_BACKEND_URL
  // in your environment to something like `https://your-render-app.onrender.com`.
  const getBackendPrefix = () => {
    try {
  ...
  useEffect(() => {
    ...
    const fetchStatus = async () => {
      try {
        ...
        const url = prefix ? `${prefix}/reload/status` : "/api/reload/status";
  ...
  const handleReloadClick = useCallback(async () => {
    ...
    try {
      const prefix = getBackendPrefix();
      const url = prefix ? `${prefix}/reload` : "/api/reload";
      await fetch(url, {
        method: "POST"
      });
    } catch (error) {
