...
let serverAvailable = true; // becomes false if we cannot spawn Python
...
function startServer() {
  ...
  try {
    ...
    // If spawn returned immediately null-ish, mark unavailable
    if (!serverProcess) {
      console.error("Failed to start serve.py: spawn returned null");
    ...
    serverProcess = null;
    serverReadyPromise = null;
    // Mark server as unavailable so callers don't block indefinitely.
    serverAvailable = false;
    // Don't rethrow here — during remote builds (or environments without
    // a Python runtime) spawning will fail. Let callers handle the
    // server-unavailable case rather than crashing the build process.
    return;
  }
...
function waitForServerReady(): Promise<void> {
  ...
  serverReadyPromise = new Promise((resolve, reject) => {
    // If we know we cannot spawn a Python server in this environment,
    // resolve quickly rather than repeatedly polling. This keeps the
    // devtools UI usable in environments like Cloudflare Pages builds.
    if (!serverAvailable) {
      console.log("serve.py unavailable in this environment, skipping readiness check");
      resolve();
      return;
    }
    ...
    const check = () => {
      ...
      const req = http.request(
        ...
        (res) => {
          if (res.statusCode && res.statusCode >= 200 && res.statusCode < 300) {
            ...
            // If we couldn't contact the server after attempts, mark
            // it unavailable and resolve so the UI doesn't hang.
            serverAvailable = false;
            console.error("serve.py did not become ready");
            resolve();
          }
        }
      );
      ...
      req.on("error", (err) => {
        ...
        if (attempts < maxAttempts) {
          ...
          // Mark unavailable and resolve to avoid blocking callers.
          serverAvailable = false;
          console.error("serve.py did not become ready (error)", err);
          resolve();
        }
      });
